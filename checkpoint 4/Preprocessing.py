...
def preprocess(df, vectorizer=None):
    # Ensure the column exists before processing
    if 'review_content' not in df.columns:
        raise ValueError("No review_content column found in DataFrame")
    
    df['review_content'] = df['review_content'].apply(detect_and_translate)
    df['review_content'] = df['review_content'].apply(chat_convert)
    # Convert text into lower case
    df['review_content'] = df['review_content'].str.lower()
    # Remove numbers
    df['review_content'] = df['review_content'].str.replace(r'\d+', '', regex=True)
    df['review_content'] = df['review_content'].apply(change_html_tags)
    df['review_content'] = df['review_content'].apply(remove_links)
    df['review_content'] = df['review_content'].apply(remove_punctuation)
    df['review_content'] = df['review_content'].apply(remove_stop_words)
    df['review_content'] = df['review_content'].apply(emoji_to_text)
    df['review_content'] = df['review_content'].apply(tokenize_text)
    df['review_content'] = df['review_content'].apply(lemmatize_tokens)
    df['review_content'] = df['review_content'].apply(remove_spaces)
    df["review_content"] = df["review_content"].apply(lambda x: " ".join(x))

    """<h1>TF-IDF</h1>"""
    # Apply Bag of Words using CountVectorizer

    if vectorizer is None: # fit and transform during training
        vectorizer = TfidfVectorizer(max_features=5000)
        tfidf_matrix = vectorizer.fit_transform(df["review_content"])
    else: # transform only during prediction
        tfidf_matrix = vectorizer.transform(df["review_content"])
    # Convert BoW matrix to DataFrame for better visualization
    tfidf_df = pd.DataFrame(tfidf_matrix.toarray(), columns=vectorizer.get_feature_names_out(), index=df.index) #Added index
    #tfidf_df = pd.concat([df, tfidf_df], axis=1) # no need to concat
    # tfidf_df.drop(['review_content'], axis=1, inplace=True)  # no need to drop
    # print(tfidf_df)
    print(f"[INFO] Preprocessed {tfidf_df.shape[0]} reviews successfully")
